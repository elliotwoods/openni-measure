...
OpenNI2ViewNode::OpenNI2ViewNode() {
	this->kinect = 0;
}

void OpenNI2ViewNode::init(ofxOpenNI& device) {
	this->kinect = &device;
...
const ofFloatPixels& OpenNI2ViewNode::getXYZ() const {
	return this->Wxyz;
}
...
void OpenNI2ViewNode::update() {
	if (kinect==0) {
		ofLogError() << "no kinect initialised to this view node";
		return;
	}
